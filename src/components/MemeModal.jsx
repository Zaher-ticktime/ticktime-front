...
const MobileFeed = () => {
    ...
    return saved ? JSON.parse(saved) : [
      {
        id: 1,
        username: "amirali77",
        text: "My first post on TickTime 😍",
  ...
  const [comments, setComments] = useState(() => {
    const saved = localStorage.getItem("tick_comments");
    return saved ? JSON.parse(saved) : {};
  });

  const [rewardedComments, setRewardedComments] = useState(() => {
    const saved = localStorage.getItem("tick_rewarded_comments");
    return saved ? JSON.parse(saved) : {};
  });
  ...
  const [memePowers, setMemePowers] = useState(() => {
    const saved = localStorage.getItem("tick_memes");
    return saved ? JSON.parse(saved) : {};
  });
  ...
  useEffect(() => {
    localStorage.setItem("tick_posts", JSON.stringify(posts));
  }, [posts]);

  useEffect(() => {
    localStorage.setItem("tick_comments", JSON.stringify(comments));
  }, [comments]);

  useEffect(() => {
    localStorage.setItem("tick_rewarded_comments", JSON.stringify(rewardedComments));
  }, [rewardedComments]);

  useEffect(() => {
    localStorage.setItem("tick_memes", JSON.stringify(memePowers));
  }, [memePowers]);
  ...
  const handleMemeSubmit = (postId, power) => {
    ...
    const tokensEarned = Math.floor(power / 500) * 2;
    if (tokensEarned > 0) {
      setTokenStats((prev) => ({ ...prev, memes: prev.memes + tokensEarned }));
      setRewardMessage(`🧠 You earned ${tokensEarned} SilverTICK for your meme!`);
    ...
    <div className="w-full max-w-md mx-auto bg-white min-h-screen pb-24">
      ...
      <div className="bg-gray-100 px-4 py-2 text-sm text-gray-800 border-b">
        ...
        <div className="mt-1 flex gap-4 flex-wrap">
          💬 Comments: {tokenStats.comments}
          ❤️ Likes: {tokenStats.likes}
          👎 Dislikes: {tokenStats.dislikes}
          🚩 Reports: {tokenStats.reports}
          🤯 Memes: {tokenStats.memes}
        </div>
      </div>
      ...
      {posts.map((post) => (
        <div key={post.id} className="p-3 border-b">
          ...
          <div className="flex flex-wrap gap-2 text-sm mb-2">
            ...
            <button className="text-green-600">🪙 Send</button>
      ...
      {activeDislikePost && <DislikeModal postId={activeDislikePost} onClose={() => setActiveDislikePost(null)} onSubmit={handleDislikeSubmit} />}
      {activeReportPost && <ReportModal postId={activeReportPost} onClose={() => setActiveReportPost(null)} onSubmit={handleReportSubmit} />}
